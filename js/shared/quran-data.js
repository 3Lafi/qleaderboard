...
// رابط الرسم الخطي الزخرفي لاسم السورة (1-114)؛ أبيض على خلفية شفافة — يصلح فقط
// فوق خلفيات داكنة/ملوّنة كهيدر اللوحة، وليس فوق البطاقات والشرائح ذات الخلفية الفاتحة
export function surahNameArtUrl(n) {
    return `/images/badges-assets/surah/${n}.png`;
}
...
/**
 * ترتيب سور الخطة حسب نوع النطاق واتجاه الحفظ.
 *
 * قاعدة الأجزاء: عند اختيار «عكس الترتيب» مع نطاق أجزاء، تُعكس الأجزاء نفسها
 * وتبقى السور داخل كل جزء بتسلسل المصحف (من بداية الجزء إلى نهايته).
 * مثال: من الجزء 29 إلى الجزء 30 مع العكس => النبأ..الناس ثم الملك..المرسلات.
 * أما نطاق السور فيُعكس ترتيب السور فيه كما هو.
 *
 * @param {number[]} surahNumbers أرقام السور ضمن النطاق
 * @param {{type?: string, direction?: string, customOrder?: number[]|null}} [options]
 * @returns {number[]}
 */
export function orderPlanSurahs(surahNumbers = [], { type = 'custom', direction = 'forward', customOrder = null } = {}) {
    const unique = [...new Set(surahNumbers)].filter(n => byNumber.has(n));
    const ascending = [...unique].sort((a, b) => a - b);

    // ترتيب مخصص يختاره المعلم بالسحب والإفلات
    if (Array.isArray(customOrder) && customOrder.length === ascending.length && ascending.every(n => customOrder.includes(n))) {
        return customOrder.slice();
    }

    // بلا عكس: يبقى الترتيب المعطى كما هو (مهم لترتيب المنهج الدراسي الرسمي)
    if (direction !== 'reverse') return unique;

    if (type === 'juz') {
        const byJuz = new Map();
        for (const n of ascending) {
            const juz = SURAHS[n - 1]?.juz ?? 0;
            if (!byJuz.has(juz)) byJuz.set(juz, []);
            byJuz.get(juz).push(n);
        }
        return [...byJuz.keys()]
            .sort((a, b) => b - a)
            .flatMap(juz => byJuz.get(juz).sort((a, b) => a - b));
    }
...
export {
    ...
    matchesQueryNameOrNumber,
    matchesStudentName,
} from './text-utils.js';
import { normalizeArabic } from './text-utils.js';
